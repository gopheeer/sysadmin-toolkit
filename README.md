...
arsenal() {
    local matrix=(
        "ripgrep (rg)        | Search & Nav  | Faster grep replacement. Respects .gitignore. | rg 'pattern'"
        "fd                  | Search & Nav  | Simpler, multi-threaded alternative to find.    | fd pattern"
        "fzf                 | Search & Nav  | General-purpose interactive fuzzy finder.       | fzf"
        "zoxide (z)          | Search & Nav  | Smart cd replacement that remembers folders.    | z folder"
        "bat                 | File Viewer   | Upgraded cat with syntax highlighting.          | bat file.txt"
        "dust                | File Viewer   | Visual, color-coded recursive disk usage tree.  | dust"
        "htop / bottom (btm) | Monitoring    | Graphical interactive system/process monitors.   | htop"
        "procs               | Monitoring    | Human-readable ps with thread tracing.          | procs"
        "fastfetch           | Monitoring    | Fast, active system information dashboard.       | fastfetch"
        "micro               | Editors       | Modern and intuitive terminal text editor.      | micro file.txt"
        "helix (hx)          | Editors       | A post-modern modal editor with built-in LSP.   | hx file.txt"
        ...
        "httpie (http)       | Network/API   | User-friendly curl alternative with JSON style. | http google.com"
        "xh                  | Network/API   | Blazing fast Rust clone of httpie.             | xh google.com"
        "trippy (trip)       | Network/API   | Combined interactive ping and traceroute UI.    | trip google.com"
        "doggo               | Network/API   | Modern DNS client (dig alternate) with DoH/DoT. | doggo google.com"
        "bandwhich           | Network/API   | Real-time utility tracking bandwidth per PID.   | sudo bandwhich"
        "jq / yq             | Data Stream   | Powerful processors for JSON/YAML streams.      | cat f.json | jq"
        "delta               | Data Stream   | Renders crisp side-by-side git diffs.           | git diff | delta"
        "tldr                | Data Stream   | Quick, real-world example cheatsheets.          | tldr tar"
        ...
        "age                 | Security      | Simple, modern file encryption tool.            | age -p f.txt > f.age"
